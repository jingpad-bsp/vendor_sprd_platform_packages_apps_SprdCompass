...
public class CompassSettings extends PreferenceActivity implements Preference.OnPreferenceChangeListener,
    ...
    public boolean onPreferenceClick(Preference preference) {
        ...
        if (KEY_SHARE_KEY.equals(preference.getKey())) {
            ...
            // bug 566705 add Muti_language resources for Android6.0
            startActivity(Intent.createChooser(intent, /* getTitle() */ getText(R.string.share_compass)));
        }
        return true;
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (preference == mBackgroundPreference) {
            ...
            /* SPRD 573291 @{ must call before setValue*/
            if(value == 4 && !showNotOwnerToast()) {
                return false;
            }
            /* @} */
            ...
            if (value == 4) {
                ...
                //SPRD: bug 570034 background was black
                intent.putExtra("applyForWallpaper", true);
    ...
    /* SPRD 573291 @{ */
    private boolean showNotOwnerToast(){
        if(UserHandle.myUserId() != UserHandle.USER_OWNER){
            /**
             * SPRD: bug843270  /vendor/sprd/platform/packages/apps/SprdCompass/res/values/strings.xml contains an error string
             * @{
             */
            Toast.makeText(this, R.string.set_background_invalid_in_guestmode , Toast.LENGTH_SHORT).show();
            /**
             * @}
             */
            return false;
    ...
    protected void onResume() {
        // TODO Auto-generated method stub
        /* SPRD:Bug569520 Modify the permission request @{ */
        ...
        /* @} */
        /* SPRD: Modified for bug 572656, update the summary after background picture deleted @{ */
        String mFilename = getSharedPreferences("test", Activity.MODE_PRIVATE).getString("path", "");
        if (!mFilename.equals("")
                && mSharedPreferences.getInt(KEY_BACKGROUND_KEY, DEFAULT_VALUE) == 4) {
            File mBgfile = new File(mFilename);
            if (!mBgfile.exists()) {
                mSharedPreferences.edit().putInt(KEY_BACKGROUND_KEY, DEFAULT_VALUE).commit();
                mBackgroundPreference.setValue(String.valueOf(DEFAULT_VALUE));
                getSharedPreferences("test", Activity.MODE_PRIVATE).edit().putString("path", "").commit();
            }
        }
        /* @} */
        if (mSharedPreferences.getInt(KEY_BACKGROUND_KEY, DEFAULT_VALUE) == 1) {
...
//                String path = cursor.getString(column_index);
                ...
                Log.d("wwg", "CompassSettings: onActivityResult:IMAGE_CODE intent= " +intent);
            ...
            Intent intent = new Intent();
            intent.putExtra("path", mFilename);
            /*SPRD 592831: update the SharedPreferences when the mFilename is null @{ */
            if (mFilename == null) {
                mSharedPreferences.edit().putInt(KEY_BACKGROUND_KEY, DEFAULT_VALUE).commit();
            }
            /* @} */
            Log.d("wwg", "CompassSettings: onActivityResult:SELECT_PIC_KITKAT intent= " +intent);
            setResult(RESULT_OK, intent);
    ...
    public static String getPath(final Context context, final Uri uri) {
        ...
        if (isKitKat && DocumentsContract.isDocumentUri(context, uri)) {
            ...
            if (isExternalStorageDocument(uri)) {
                ...
                /* SPRD 567201 @{ Now,just think about SD-card condition,if support OTG,need to modify,again*/
                }else {
    ...
    public static String getDataColumn(Context context, Uri uri, String selection, String[] selectionArgs) {
        ...
        try {
            ...
            Toast.makeText(context, R.string.failed_option , Toast.LENGTH_SHORT).show();
        } finally {
            if (cursor != null)
                cursor.close();
        }
